...
## vuex-modules
Vuex 的 modules（模块）是为了拆分复杂的 store 而设计的功能，核心作用是让代码结构更清晰。

- **基本功能**：每个模块可独立包含 state、mutations、actions、getters，还能嵌套子模块。
- **命名空间**：通过 `namespaced: true` 开启，避免不同模块的方法重名冲突，调用时需带模块名（如 `模块名/方法名`）。
- **优势**：按功能拆分代码（如用户模块、商品模块），方便维护；隔离不同模块的状态和方法，适合大型应用。
